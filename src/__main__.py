...
def main():
    ...
    parser.add_argument("--library_id", help="library id of the account") # 9467597
    parser.add_argument("--library_type", default="user", help="Accessing user library or group library?") 
    parser.add_argument("--api_key", help="API Key") #7dqMME5kwR9nMYbGZlXQCc4H
    ...
    ds.save_to_disk(args.path)
    
    # for paper in data:
    #     title = paper['info']["title"]
    #     print(title)
    #     info = s2_title_search(title)
    #     print(info)
    #     ids.append(info['paperId'])
    ...
    # dois = [paper['info']['doi'] for paper in data if paper['info']['type'] != 'Editorship']
        
    # print(dois)
    # for paper in data:
    #     if paper['info']['type'] == 'Editorship':
    #         continue
    #     print(paper['info']["title"])
    #     url = paper['info']['ee']
    #     if args.abstract:
    #         abstract = get_abstract(args.conf, url)
    #         print(abstract)
